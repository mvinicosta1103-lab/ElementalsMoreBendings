...
/**
 * Entidade puramente visual de UM espinho de cristal. Antes, {@link
 * CrystalSpikeAbility} só trocava a textura do chão pra {@code
 * amethyst_block} (mais os block-particles de {@code applySpikes}) --
 * lia como "piso diferente", não como um espinho de verdade brotando do
 * chão. Essa entidade dá o "corpo" pro {@link CrystalSpikeVisualEntityRenderer}
 * desenhar uma farpa de ametista de verdade em cima de cada posição, exatamente
 * o mesmo esquema que {@code MagmaSpikeVisualEntity} já usa pra {@code
 * magmaSpike} (sem física, sem gravidade, sem colisão -- só existência +
 * timer pro renderer ler).
 * <p>
 ...
 * CrystalSpikeAbility#RETRACT_AFTER_TICKS} que a ability já usa pra
 * reverter o bloco (ver {@link CrystalSpikeManager}) -- assim o modelo
 ...
 */
public class CrystalSpikeVisualEntity extends Entity {

    private static final EntityDataAccessor<Integer> SEED =
            SynchedEntityData.defineId(CrystalSpikeVisualEntity.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Integer> LIFETIME_TICKS =
            SynchedEntityData.defineId(CrystalSpikeVisualEntity.class, EntityDataSerializers.INT);

    public CrystalSpikeVisualEntity(EntityType<CrystalSpikeVisualEntity> type, Level level) {
        super(type, level);
        this.noPhysics = true;
    ...
    public CrystalSpikeVisualEntity(Level level, BlockPos growPos, int lifetimeTicks, int seed) {
        this(ModEntities.CRYSTAL_SPIKE_VISUAL.get(), level);
     ...
     * já adicionada ao nível -- helper usado por {@link CrystalSpikeAbility},
     * mesmo padrão de {@code MagmaSpikeVisualEntity#spawn}.
     */
    public static CrystalSpikeVisualEntity spawn(ServerLevel level, BlockPos growPos, int lifetimeTicks) {
        int seed = level.random.nextInt(Integer.MAX_VALUE);
        CrystalSpikeVisualEntity spike = new CrystalSpikeVisualEntity(level, growPos, lifetimeTicks, seed);
