...
/**
 * "mudWall" — nova habilidade da árvore de Mud (ver {@link MudElement}),
 * aninhada como filha de {@code mudTrap}. Levanta uma parede sólida de
 * lama (3 de largura x 3 de altura) na frente do caster por um tempo
 * curto -- bloqueia flechas e movimento, igual a {@code AbilityEarthWall}
 * do mod base, só que temporária e feita de {@link Blocks#MUD}.
 *
 * Mesmo padrão de {@link com.elementals.morebendings.bending.earthsubbendings.crystal.CrystalWallAbility}:
 * cada bloco da parede é um {@link EarthBlockEntity} flutuante subindo do
 * chão da própria coluna até sua altura final, sem nunca virar bloco de
 * verdade no mundo -- por isso não deixa buraco nem precisa restaurar
 * terreno depois (ver {@link MudWallManager}).
 *
 * Sem limite de paredes simultâneas -- cada cast sobe a sua própria,
 * independente das que já estão de pé.
 */
public class MudWallAbility implements Ability {
    ...
    public void onCall(Bender bender, long heldTimeMs) {
        ...
        if (!entities.isEmpty()) {
            MudWallManager.registerWall(player, entities, DURATION_TICKS);
            level.playSound(null, center, SoundEvents.MUD_PLACE, SoundSource.PLAYERS, 1.0f, 0.9f);
        }
    ...
    /**
     * Sobe uma coluna de {@code HEIGHT} blocos de lama, uma entidade por
     * altura, todas partindo do nível do chão da coluna e subindo até a
     * posição final -- igual {@code CrystalWallAbility#raiseColumn}.
     */
    private void raiseColumn(ServerLevel level, Player player, BlockPos column, LinkedList<EarthBlockEntity> entities) {
        for (int h = 0; h < HEIGHT; h++) {
            ...
            // nunca vira bloco de verdade no mundo -- desmancha quando a parede acabar
